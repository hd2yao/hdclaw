...
本文档适用于当前仓库默认的 `openclaw-official` Docker 部署。

命令约定：

- 默认在仓库根目录执行
- 需要调用 `openclaw` CLI 时，统一通过 `docker compose -f containers/openclaw-official/docker-compose.yml exec openclaw ...`
- 不要假定宿主机已经安装 `openclaw`
...
- 本仓库的 `make sync` 不管理 Docker 实例里的 Telegram bot token。
- Telegram token 属于运行时密钥，实际写入位置是容器内 `/home/node/.openclaw/openclaw.json`。
- 因此，绑定 Telegram 的标准入口是容器内执行 `openclaw channels add ...`，不是手改仓库配置。
...
如果当前容器还没有绑定过 Telegram token，执行：

```bash
docker compose -f containers/openclaw-official/docker-compose.yml exec openclaw \
...
```bash
docker compose -f containers/openclaw-official/docker-compose.yml exec openclaw openclaw channels status --probe
docker compose -f containers/openclaw-official/docker-compose.yml exec openclaw openclaw health
docker compose -f containers/openclaw-official/docker-compose.yml exec openclaw openclaw pairing list telegram --json
make docker-logs
```
...
2. 在容器内执行 `openclaw channels add --channel telegram --token '<TELEGRAM_BOT_TOKEN>'`
3. 在容器内执行 `openclaw gateway restart`
4. 在容器内执行 `openclaw channels status --probe`
5. 在 Telegram 私聊 bot 发 `/start`
6. 在容器内执行 `openclaw pairing list telegram --json`
7. 在容器内执行 `openclaw pairing approve telegram <PAIRING_CODE> --notify`
8. 再发一条测试消息确认回复
