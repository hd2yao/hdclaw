...
MOCK_UPSTREAM_MODE="stream-toolcall" \
...
REQ='{"model":"mock-model","stream":true,"messages":[{"role":"user","content":"weather"}]}'
...
if ! grep -q '"tool_calls"' "$BODY_FILE"; then
  echo "[toolcall-transform-stream] expected tool_calls in stream output" >&2
  exit 1
fi

if grep -q '<tool_call>' "$BODY_FILE"; then
  echo "[toolcall-transform-stream] raw <tool_call> leaked to client output" >&2
  exit 1
fi

echo "[toolcall-transform-stream] passed"
