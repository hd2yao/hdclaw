...
echo "[test-config] case6: switching to disabled web_search should clear provider/apiKey"
OPENCLAW_ENV_FILE="$ENV_OK" OPENCLAW_CONFIG_TARGET="$TARGET" OPENCLAW_BACKUP_DIR="$BACKUP_DIR" \
  bash "$ROOT_DIR/scripts/sync-config.sh" >/dev/null
...
jq -er '.tools.web.search.provider == null' "$TARGET" >/dev/null || {
  echo "[test-config] expected web_search provider to be cleared when disabled" >&2
  exit 1
}

jq -er '.tools.web.search.apiKey == null' "$TARGET" >/dev/null || {
  echo "[test-config] expected web_search apiKey to be cleared when disabled" >&2
  exit 1
}
