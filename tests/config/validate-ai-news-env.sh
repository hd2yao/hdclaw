...
ENV_BAD_TIMEOUT="$TMP_DIR/.env.bad-timeout"
...
echo "[test-ai-news-env] case2b: invalid AI_NEWS_TIMEOUT_SECONDS should fail"
if OPENCLAW_ENV_FILE="$ENV_BAD_TIMEOUT" bash "$ROOT_DIR/scripts/setup-ai-news-daily-cron.sh" --dry-run >/dev/null 2>&1; then
  echo "[test-ai-news-env] expected timeout validation failure but got success" >&2
  exit 1
fi
