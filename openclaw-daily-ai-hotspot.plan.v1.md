...
1. Cron job 触发。
2. Agent 按模板执行：
  - 从英文优先来源检索近 24h AI 信息；不足 10 条回溯到 48h 补齐。
  - 去重与热度排序后确定 Top10。
...
- `/Users/dysania/program/openclaw/scripts/ai-news-daily-prompt.template.md`
  - 注入变量模板：`{OBSIDIAN_DIR}` `{TELEGRAM_TARGET}` `{TZ}` `{DATE}`
...
2. 回退仓库改动
- 将 `skills/catalog.yaml` 中 `ai-news-daily` 设为 `enabled: false`
- 移除新增 Makefile 目标与 README 段落
- 保留历史文档文件（用于审计）
