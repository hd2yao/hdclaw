...
- Promise-only replies are forbidden after those triggers. Invalid examples: `我继续推进`, `我这就去做`, `我按这个方案开工`, `稍等我整理`, `已开工，我会按这个方案直接落地并启用`.
...
- A reply that claims execution started is invalid unless this turn already contains both:
  - one real tool action (`exec`/`process`/file read/test/script run)
  - one task receipt update via `openclaw-active-task.sh set` or `clear`
