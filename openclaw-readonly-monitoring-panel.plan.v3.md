...
- 前端交付流程改为 `Figma 线框 -> Figma 高保真 -> 按批准后的 Figma 节点实现`，不再直接跳过设计稿编码。
- 支持多个 OpenClaw 节点实例接入，并按节点展示下属 agents。
- 支持 1 到 2 秒级的状态刷新体验。
- 支持短期历史，MVP 先明确为最近 `1h` 与 `24h` 两档窗口。
- 展示任务时间线、当前阶段、开始时间、持续时间和最近进展摘要。
- 不展示模型原始思考、完整会话正文或工具调用参数。
- Figma 关键页面固定为 4 个：`Dashboard Overview`、`Node Detail`、`Agent Work Detail / Timeline`、`Alerts / Events`。
...
- 设计到代码链路改为 Figma-first：
  - 先在 Figma 完成关键页线框与高保真稿。
  - 再用 Figma MCP 基于已批准节点的 `nodeId` / screenshot / design context 实现前端。
  - 代码实现必须和最终批准的 Figma 节点映射表保持一致。
...
- 第 1 步：先在 Figma 产出 4 个关键页的线框：
  - `Dashboard Overview`
  - `Node Detail`
  - `Agent Work Detail / Timeline`
  - `Alerts / Events`
- 第 2 步：线框确认信息架构、导航层级、关键交互和非技术可读性，不通过前不进入高保真。
- 第 3 步：在线框确认后产出高保真稿，至少覆盖 `Desktop`、`Mobile` 两种 frame；`Dashboard Overview` 与 `Agent Work Detail / Timeline` 额外补 `Tablet`。
- 第 4 步：实现阶段严格基于最终批准的 Figma 节点实现，记录页面 frame URL、组件 node URL 与 node ID，供 Figma MCP 拉取上下文。
...
- `Node Detail`
  - 以单节点为焦点展示完整基础信息、资源趋势、agent 分布和最近异常事件。
  - 支持从 overview 进入，并保留返回 overview 的路径。
- `Agent Work Detail / Timeline`
  - 聚焦单个 agent 的当前任务、阶段、持续时长、最近进展摘要与 `1h / 24h` 时间线。
  - 对非技术用户突出人类可读摘要，对运维用户保留状态、时间和节点上下文。
- `Alerts / Events`
  - 展示异常/事件列表，包含 `severity`、节点、agent、摘要、发生时间、是否恢复。
  - 支持基础筛选：`all / warning / critical / recovered`、节点过滤、时间窗口切换。
  - 右侧或抽屉提供 alert detail，展示最近上下文和关联节点/agent。
...
- 点击告警条目时，展开或切换 alert detail，显示关联节点、agent、最近上下文与恢复状态。
- 当 WebSocket 断开时，保留最后一次快照并在顶栏显式显示连接状态，不清空页面。
- 当节点暂无 agents、agent 无历史、节点离线、接口报错、告警列表为空时，分别提供明确空态和错误态。

### Responsive Variants

- Figma 中采用“独立 frame + auto-layout/constraints 组合”方案：
  - `Desktop`、`Tablet`、`Mobile` 分别建独立 frame，作为实现的视觉基准。
  - frame 内部使用 auto-layout 和 constraints 表达可伸缩关系，但不以单 frame 拉伸代替多断点设计。
- 页面覆盖要求：
  - 4 个关键页都必须有 `Desktop` frame。
  - `Dashboard Overview`、`Agent Work Detail / Timeline` 必须额外提供 `Tablet` frame。
  - 4 个关键页都必须有 `Mobile` frame。
- 布局要求：
...
- 桌面端：`Alerts / Events` 页面显示 warning、critical、recovered 三类事件，并可展开 detail。
- 平板端：摘要与节点详情堆叠时不出现横向滚动。
- 手机端：节点切换、agent 列表和 timeline 抽屉可正常操作。
- 手机端：告警筛选和 detail 抽屉可正常操作。
- 断线态：顶栏显示 WebSocket closed，页面保留最后快照。
- 超时态：显示 skeleton -> timeout fallback -> 手动重试入口。

### Design Handoff

- 设计资产统一放在一个 Figma 文件中，至少包含 4 个 page：
  - `Wireframes`
  - `Hi-Fi`
  - `Components`
  - `Tokens`
- 线框确认后，把最终 Figma 文件 URL 与关键 frame URL 回写到设计文档中，避免实现期口头同步。
- 建立组件映射表，最少包含：
  - `Figma Frame / Component Name`
  - `Node ID`
  - `Code Component Path`
  - `Status`（new / reuse / adapted）
- 从 Figma 提取并固化设计 token：
  - colors
  - spacing
  - radius
  - shadow
  - typography
  - breakpoints
- 响应式变体必须在 Figma 中明确到 frame 级，而不是只写文案说明。
- 实现阶段使用 Figma MCP：
  - `get_design_context` 拉设计上下文
  - `get_screenshot` 做视觉核对
  - 若后续需要 code connect，再补节点到组件映射，不在本轮 MVP 先决条件中强绑
...
- 优先复用现有组件：
  ...
  - `frontend/src/components/nodes/NodeOverview.tsx`
  - `frontend/src/components/charts/ResourceChart.tsx`
  - `frontend/src/components/agents/AgentTable.tsx`
  - `frontend/src/components/agents/SessionHistoryPanel.tsx`
- 新增或显著改造组件：
  - `frontend/src/components/summary/GlobalSummary.tsx`
  - `frontend/src/components/states/DashboardSkeleton.tsx`
  - `frontend/src/app/AlertsPage.tsx`
  - `frontend/src/components/alerts/AlertFilterBar.tsx`
  - `frontend/src/components/alerts/AlertDetailPanel.tsx`
  - `frontend/src/components/states/EmptyStatePanel.tsx`
- 等 Figma 产出后，在设计文档中补每个新组件对应的 `nodeId` 和 frame URL。
...
### Task 5: 产出 Figma 线框、高保真和设计交接说明

**Files**
- Create: [docs/plans/2026-03-17-openclaw-monitoring-panel-figma-design.md](/Users/dysania/program/openclaw/docs/plans/2026-03-17-openclaw-monitoring-panel-figma-design.md)
- Modify: [frontend/README.md](/Users/dysania/program/openclaw/projects/openclaw-dashboard/frontend/README.md)

**Outcome**
- 先完成 4 个关键页线框并确认，再补高保真。
- 设计文档里记录 Figma 文件 URL、关键 frame URL、node ID、组件映射表和 token 摘要。
- 实现阶段只按批准后的 Figma 版本落代码，不再脱稿发挥。

### Task 6: 按 Figma 实现双层首页、明细页与告警页组件改造
...
- Create: [frontend/src/app/AlertsPage.tsx](/Users/dysania/program/openclaw/projects/openclaw-dashboard/frontend/src/app/AlertsPage.tsx)
- Create: [frontend/src/components/alerts/AlertFilterBar.tsx](/Users/dysania/program/openclaw/projects/openclaw-dashboard/frontend/src/components/alerts/AlertFilterBar.tsx)
- Create: [frontend/src/components/alerts/AlertDetailPanel.tsx](/Users/dysania/program/openclaw/projects/openclaw-dashboard/frontend/src/components/alerts/AlertDetailPanel.tsx)
- Create: [frontend/src/components/states/EmptyStatePanel.tsx](/Users/dysania/program/openclaw/projects/openclaw-dashboard/frontend/src/components/states/EmptyStatePanel.tsx)

**Outcome**
- 页面真正变成“业务摘要 + 运维明细”的双层监控面板，并补齐节点详情、agent 明细和告警页，而不是单纯节点卡片页。
...
- Figma 设计校验
  - 在实现前使用 Figma MCP 校验关键页设计文档和节点引用是否完整。
  - 覆盖 `Dashboard Overview`、`Node Detail`、`Agent Work Detail / Timeline`、`Alerts / Events` 4 个页面。
  - 至少对批准版本执行 `get_design_context` 与 `get_screenshot`，确认 node ID、frame URL 与视觉稿一致。
...
- 视觉验收
  - 在实现阶段使用浏览器验证桌面、平板、手机三组截图点，并与 Figma 批准稿做截图比对。
  - 至少覆盖健康节点、降级节点、断线态、超时态、空 timeline、告警 detail 六类场景。
...
- 如果没有 Figma 约束，前端容易滑回通用 admin 风格，和“业务摘要 + 运维明细”的预期脱节
  - Mitigation: 前端改为 Figma-first 流程，先确认线框和高保真，再基于批准 node ID 实现和验收。
